...
/**
 * Record reminder send state for pro reminders.
 * Unlike core appointment reminders, these may not be tied to an appointment
 * and may use reminderType values outside the core "24h"/"1h"/"birthday" union.
 */
export const recordProReminderSent = internalMutation({
  args: {
    patientId: v.id("patients"),
  ...
  handler: async (ctx) => {
    ...
    for (const team of teams) {
      ...
      for (const patient of patients) {
        ...
        if (result.ok) {
          await ctx.runMutation(internal.proReminders.recordProReminderSent, {
            patientId: patient._id,
