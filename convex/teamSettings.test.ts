...
describe("teamSettings.update", () => {
  ...
  it("rejects changing Public Link ID after it is set", async () => {
    const t = convexTest(schema, modules);
    await seedTeamAndUser(t);
    const asSmith = t.withIdentity({ email: "smith@acme.test" });

    await asSmith.mutation(api.teamSettings.update, { entrySlug: "acme" });

    await expect(
      asSmith.mutation(api.teamSettings.update, { entrySlug: "acme-new" }),
    ).rejects.toThrow("Public Link ID is locked");
  });

  it("normalizes Public Link ID to lowercase + hyphenated characters", async () => {
    const t = convexTest(schema, modules);
    await seedTeamAndUser(t);
    const asSmith = t.withIdentity({ email: "smith@acme.test" });

    await asSmith.mutation(api.teamSettings.update, { entrySlug: "Acme Dental!!" });
    const settings = await asSmith.query(api.teamSettings.get, {});

    expect(settings.entrySlug).toBe("acmedental");
  });
